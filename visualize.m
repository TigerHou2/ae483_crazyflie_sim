function visualize(T, O, HY, HP, HR, C, moviefile)
%
%   t           time (1xM)
...
% a triangular mesh)
[pQuad_InBody0, fQuad0] = GetQuadModel('quadmodel.mat');
pQuad_InBody(1:length(T)) = {pQuad_InBody0};
fQuad(1:length(T)) = {fQuad0};
...
pRoomFrame_InRoom0 = [zeros(3,1) eye(3)];
pBodyFrame_InBody0 = [zeros(3,1) eye(3)];

pRoomFrame_InRoom(1:length(T)) = {pRoomFrame_InRoom0};
pBodyFrame_InBody(1:length(T)) = {pBodyFrame_InBody0};

pQuad_InRoom = cell(1,length(T));
pBodyFrame_InRoom = cell(1,length(T));
...
tic;
axSet = false;
axBounds = [0,0,0,0,0,0];
allbreak = false(1,length(T));
...
while (1)
    ...
    for k = 1:length(T)
        ...
        % Update geometry
        [pQuad_InRoom{k}, pBodyFrame_InRoom{k}] = ...
            UpdateGeometry(t(i), o(:, i), hy(i), hp(i), hr(i), ...
                           pQuad_InBody{k}, pBodyFrame_InBody{k});

        % Update figure
        if not(axSet)
            axBounds0 = [min(o(1,:))-0.7, max(o(1,:))+0.7, ...
                         min(o(2,:))-0.7, max(o(2,:))+0.7, ...
                         min(o(3,:))-0.7, max(o(3,:))+0.7];
            axBounds = max( [-1,1,-1,1,-1,1] ...
                          .*[axBounds;axBounds0] ) ...
                          .*[-1,1,-1,1,-1,1];
        end
        
        hold on
        fig{k} = UpdateFigure(fig{k}, axBounds, t(i), o(:, i), ...
                                pQuad_InRoom{k}, fQuad{k}, ...
                                pRoomFrame_InRoom{k}, ...
                                pBodyFrame_InRoom{k}, ...
                                C{k},k);
...
R_1_to_0=[cos(hy)*cos(hp) cos(hy)*sin(hp)*sin(hr)-sin(hy)*cos(hr) cos(hy)*sin(hp)*cos(hr)+sin(hy)*sin(hr);...
    sin(hy)*cos(hp) sin(hy)*sin(hp)*sin(hr)+cos(hy)*cos(hr) sin(hy)*sin(hp)*sin(hr)-cos(hy)*sin(hr);...
    -sin(hp) cos(hp)*sin(hr) cos(hp)*cos(hr)];

pQuad_InRoom = o + R_1_to_0 * pQuad_InBody;
pBodyFrame_InRoom = 0 + R_1_to_0 * pBodyFrame_InBody;
...
function [fig] = UpdateFigure(fig, axBounds, t, o, pQuad_InRoom, fQuad, pRoomFrame_InRoom, pBodyFrame_InRoom, c, idx)
...
if isempty(fig)
    ...
    % Draw the floor
    fig.floor = rectangle('position', [-5 -15 20 20], ...
                          'facecolor', 0.9*[1 1 1]);
...
%                          15, 'k', 'filled');
    ...
    % Draw the quadrotor
    fig.quad = DrawMesh([], pQuad_InRoom, fQuad, c, 0.6);
    
    % Draw trace of position
    fig.o = DrawTrace([], nan(3,1), c, 50);
